export default class ValidatorUtils {
  ...
  // 车牌正则
  private static REGEXP_LICENSE_PLATE = /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领]{1}[A-Z]{1}[A-Z0-9]{4}[A-Z0-9挂学警港澳]{1}$/
  // 身份证正则
  private static REGEXP_ID_CARD = /^\d{6}(18|19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/i
  // 经度正则
  private static REGEXP_LONGITUDE = /^-?(((1?[0-7]?[0-9]?)|(0?[0-9]?[0-9]))(([.][0-9]{3,6})?)|180(([.][0]{3,6})?))$/
  // 维度正则
  private static REGEXP_LATITUDE = /^-?((0|[1-8]?[0-9]?)(([.][0-9]{3,6})?)|90(([.][0]{3,6})?))$/
   ...
   */
  public isLicense(license: string): boolean {
  ...
  public isValidID(id: string): boolean {
    ...
    if (city[id.substring(0, 2)]) {
      if (ValidatorUtils.REGEXP_ID_CARD.test(id)) {
        ...
        let sum = 0
        for (let i = 0; i < 17; i++) {
          sum += ((idWordsList[i] as unknown) as number) * factor[i]
        }
   ...
   * @param  coords 经纬度
   */
  public isCoords(coords: string): boolean {
    let list = coords.split(',')
    return (
      ValidatorUtils.REGEXP_LONGITUDE.test(list[0]) &&
